...
func (s *Service) cachedVerification(id string) (VerificationReport, error, bool) {
	s.verificationMu.Lock()
	defer s.verificationMu.Unlock()
	entry, ok := s.verificationCache[id]
	return entry.report, entry.err, ok
}

func (s *Service) cacheVerification(id string, report VerificationReport, err error) (VerificationReport, error) {
	s.verificationMu.Lock()
	s.verificationCache[id] = verificationCacheEntry{report: report, err: err}
	s.verificationMu.Unlock()
	return report, err
}

func (s *Service) Verification(id string) (VerificationReport, error) {
	if report, err, ok := s.cachedVerification(id); ok {
		return report, err
	}
	...
	if !ok {
		...
		return s.cacheVerification(id, r, fmt.Errorf("case not found"))
	}
	if c.Evidence != nil {
	...
	if err := audit.VerifyManifest(c, s.Store.Events(id)); err != nil {
		...
		return s.cacheVerification(id, r, err)
	}
	r.Valid = true
	return s.cacheVerification(id, r, nil)
}
