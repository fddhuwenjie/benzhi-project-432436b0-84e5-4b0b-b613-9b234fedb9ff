...
func New(dir string) (*Store, error) {
	...
	if err := recoverSnapshotTemps(dir); err != nil {
		return nil, err
	}
...
func recoverSnapshotTemps(dir string) error {
	entries, err := os.ReadDir(dir)
	if err != nil {
		return err
	}
	for _, entry := range entries {
		if entry.IsDir() || !strings.HasSuffix(entry.Name(), ".snapshot.tmp") {
			continue
		}
		caseID := strings.TrimSuffix(entry.Name(), ".snapshot.tmp")
		if err := os.Rename(filepath.Join(dir, entry.Name()), filepath.Join(dir, caseID+".json")); err != nil {
			return err
		}
	}
	return nil
